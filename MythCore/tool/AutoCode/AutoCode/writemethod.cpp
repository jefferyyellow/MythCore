	...
	for (int i = 0; i < (int)rVariableList.size(); ++i)
	{
		...
		if (NULL != pDefaultValue)
		{
			switch (rVariableList[i]->getArrayDimension())
			{
				case 0:
				{
					writeBuiltInType(rVariableList[i], pFile, nSpaceNum, nCount);
					break;
				}
				case 1:
				{
					...
					{
						writeBuiltInArray(rVariableList[i], pFile, nSpaceNum, nCount);
					}
					break;
				}
				case 2:
				{
					...
					{
						writeBuiltInArray2(rVariableList[i], pFile, nSpaceNum, nCount);
					}
					break;
				}
				default:
...
// 写非数组的内置类型
void CParseHeader::writeBuiltInType(CVariable* pVariable, FILE* pFile, int nSpaceNum, int &rCount)
{
...
// 写内置数组
void CParseHeader::writeBuiltInArray(CVariable* pVariable, FILE* pFile, int nSpaceNum, int &rCount)
{
	if (NULL == pVariable || NULL == pFile)
	{
		return;
...
/// 写二维内置类型数组
void CParseHeader::writeBuiltInArray2(CVariable* pVariable, FILE* pFile, int nSpaceNum, int &rCount)
{
