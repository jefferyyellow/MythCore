...
int CMD5::compute(const void *pInput, int32 nInputLength, void *pDigest)
{
	int nResult  = -1;

	const uint8 *pucInput = (const uint8 *)pInput;
	uint8       *pucDigest = (uint8 *)pDigest;
...
void CMD5::MD5Update(MD5Context *s, const uint8 *p, uint32 len)
{
	uint8 *q    = (uint8 *) p;
	uint32 lenw = len;
	uint32 wordblock[16];
	...
	const int BskSIZE = 64;
	if (s->bskused + len < ( unsigned int )BskSIZE) {
		/*
		 * Trivial case: just add to the block.
		 */
		memcpy(s->block + s->bskused, q, len);
		s->bskused += len;
	} else {
		/*
		 * We must complete and process at least one block.
		 */
		while (s->bskused + len >= ( unsigned int )BskSIZE) {
			memcpy(s->block + s->bskused, q, BskSIZE - s->bskused);
			q += BskSIZE - s->bskused;
			len -= BskSIZE - s->bskused;
...
void CMD5::MD5Final(uint8 output[], MD5Context *s)
{
	int32  i;
	uint32 pad;
	uint8  c[64];
...
void CMD5::MD5_Block(MD5_Core_State *s, uint32 *block)
{
	uint32 a, b, c, d;
	a = s->h[0];
	b = s->h[1];
