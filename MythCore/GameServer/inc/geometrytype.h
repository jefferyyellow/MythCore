...
public:
	// 默认构造一个 CMythSize( 0, 0 ) 对象
	CMythSize( ) : mCX( 0 ), mCY( 0 )						{	}

	// 通过两个整数构造一个 CMythSize 对象
...
public:
	// 默认构造一个 CMythPoint( 0, 0 ) 对象
	CMythPoint( ) { }//: mX( 0 ), mY( 0 )						{	}
	// 通过两个整数构造一个 CMythPoint 对象
...
public:
	...
	// 该操作返回该CMythPoint对象减去一个CMythPoint对象后，构造的CMythPoint拷贝 */
	CMythPoint operator - ( const CMythPoint& rPoint ) const
		{ return CMythPoint( mX - rPoint.mX, mY - rPoint.mY ); }

	// 该操作返回该CMythPoint对象减去一个CMythSize对象后，构造的CMythPoint拷贝
	CMythPoint operator - ( const CMythSize& rSize ) const
		{ return CMythPoint( mX - rSize.mCX, mY - rSize.mCY ); }

	// 该操作返回CMythRect对象减去该CMythPoint对象后，构造的CMythRect拷贝
	CMythRect operator - ( const CMythRect& rRect ) const;

	// 该操作返回该CMythPoint对象取反后，构造的CMythPoint拷贝
	CMythPoint operator - ( ) const
		{ return CMythPoint( -mX, -mY ); }

	// 该操作返回该CMythPoint对象加上一个CMythPoint对象后，构造的CMythPoint拷贝
	CMythPoint operator + ( const CMythPoint& rPoint ) const
		{ return CMythPoint( mX + rPoint.mX, mY + rPoint.mY ); }

	// 该操作返回该CMythPoint对象加上一个CMythSize对象后，构造的CMythPoint拷贝
	CMythPoint operator + ( const CMythSize& rSize ) const
		{ return CMythPoint( mX + rSize.mCX, mY + rSize.mCY ); }

	// 该操作返回CMythRect对象加上该CMythPoint对象后，构造的CMythRect拷贝
	CMythRect operator + ( const CMythRect& rRect ) const;
};
...
public:
	// 默认构造一个 CMythRect( 0, 0, 0, 0 ) 对象
	CMythRect( )	{ normalizeRect( ); }

	// 通过4个整数构造 CMythRect 对象, 
...
public:
	...
	// 缩小该 CMythRect 对象，缩小的尺寸由 rRect 指定
	CMythRect operator - ( const CMythRect& rRect ) const
	{
		CMythRect tRectResult = *this;
	...
	// 扩大该 CMythRect 对象，扩大的尺寸由 rRect 指定
	CMythRect operator + ( const CMythRect& rRect ) const
	{
		CMythRect tRectResult = *this;
